...
class TestRgbToHslConversion(unittest.TestCase):
    def test_basic_colors(self):
        ...
        for idx, (rgb, expected_hsl) in enumerate(test_cases):
            with self.subTest(i=idx):
                ...
                self.assertTrue(torch.allclose(hsl, expected_hsl, atol=1e-4), msg=f"Failed for RGB input: {rgb}")
    ...
    def test_hue_wraparound(self):
        ...
        self.assertTrue(torch.allclose(hsl_red, expected_hsl_red, atol=1e-4), msg="Hue wraparound failed for pure red.")
    ...
    def test_zero_saturation(self):
        ...
        for idx, (rgb, expected_hsl) in enumerate(grayscales):
            with self.subTest(i=idx):
                hsl = rgb_tensor_to_hsl_tensor(rgb)
                self.assertTrue(torch.allclose(hsl, expected_hsl, atol=1e-4), msg=f"Failed for grayscale RGB input: {rgb}")
    ...
    def test_non_float_input(self):
        ...
        self.assertTrue(torch.allclose(hsl_normalized, expected_hsl, atol=1e-4), msg="Integer input was not correctly normalized when normalize=True.")
    ...
    def test_random_values(self):
        ...
        self.assertTrue(torch.all((hsl_random[..., 0] >= 0) & (hsl_random[..., 0] < 1)), msg="Hue values out of range [0,1).")
        self.assertTrue(torch.all((hsl_random[..., 1] >= 0) & (hsl_random[..., 1] <= 1)), msg="Saturation values out of range [0,1].")
        self.assertTrue(torch.all((hsl_random[..., 2] >= 0) & (hsl_random[..., 2] <= 1)), msg="Lightness values out of range [0,1].")
    ...
    def test_batch_processing(self):
        ...
        # Check the shape
        self.assertEqual(hsl_batch.shape, rgb_batch.shape, msg="Output HSL tensor shape mismatch for batch input.")

        # Check value ranges
        self.assertTrue(torch.all((hsl_batch[..., 0] >= 0) & (hsl_batch[..., 0] < 1)), msg="Hue values out of range [0,1) in batch input.")
        self.assertTrue(torch.all((hsl_batch[..., 1] >= 0) & (hsl_batch[..., 1] <= 1)), msg="Saturation values out of range [0,1] in batch input.")
        self.assertTrue(torch.all((hsl_batch[..., 2] >= 0) & (hsl_batch[..., 2] <= 1)), msg="Lightness values out of range [0,1] in batch input.")
    ...
    def test_high_resolution(self):
        ...
        try:
            ...
            self.assertEqual(hsl_high_res.shape, high_res.shape, msg="Output HSL tensor shape mismatch for high-resolution input.")
            # Check value ranges
            self.assertTrue(torch.all((hsl_high_res[..., 0] >= 0) & (hsl_high_res[..., 0] < 1)), msg="Hue values out of range [0,1) in high-resolution input.")
            self.assertTrue(
                torch.all((hsl_high_res[..., 1] >= 0) & (hsl_high_res[..., 1] <= 1)), msg="Saturation values out of range [0,1] in high-resolution input."
            )
            self.assertTrue(
                torch.all((hsl_high_res[..., 2] >= 0) & (hsl_high_res[..., 2] <= 1)), msg="Lightness values out of range [0,1] in high-resolution input."
            )
