...
class TestRgbToHsvConversion(unittest.TestCase):
    def test_basic_colors(self):
        ...
        for idx, (rgb, expected_hsv) in enumerate(test_cases):
            with self.subTest(i=idx):
                ...
                self.assertTrue(torch.allclose(hsv, expected_hsv, atol=1e-4), msg=f"Failed for RGB input: {rgb}")
    ...
    def test_hue_wraparound(self):
        ...
        self.assertTrue(torch.allclose(hsv_red, expected_hsv_red, atol=1e-4), msg="Hue wraparound failed for pure red.")
    ...
    def test_zero_saturation(self):
        ...
        for idx, (rgb, expected_hsv) in enumerate(grayscales):
            with self.subTest(i=idx):
                hsv = rgb_tensor_to_hsv_tensor(rgb)
                self.assertTrue(torch.allclose(hsv, expected_hsv, atol=1e-4), msg=f"Failed for grayscale RGB input: {rgb}")
            ...
            with self.subTest(i=idx):
                if should_raise:
                    ...
                    try:
                        ...
                        self.assertEqual(hsv.shape, invalid_tensor.shape, msg="Output HSV tensor shape mismatch for valid batch input.")
                    except ValueError:
    ...
    def test_non_float_input(self):
        ...
        self.assertTrue(torch.allclose(hsv_normalized, expected_hsv, atol=1e-4), msg="Integer input was not correctly normalized when normalize=True.")
    ...
    def test_random_values(self):
        ...
        self.assertTrue(torch.all((hsv_random[..., 0] >= 0) & (hsv_random[..., 0] < 1)), msg="Hue values out of range [0,1).")
        self.assertTrue(torch.all((hsv_random[..., 1] >= 0) & (hsv_random[..., 1] <= 1)), msg="Saturation values out of range [0,1].")
        self.assertTrue(torch.all((hsv_random[..., 2] >= 0) & (hsv_random[..., 2] <= 1)), msg="Value values out of range [0,1].")
    ...
    def test_batch_processing(self):
        ...
        # Check the shape
        self.assertEqual(hsv_batch.shape, rgb_batch.shape, msg="Output HSV tensor shape mismatch for batch input.")

        # Check value ranges
        self.assertTrue(torch.all((hsv_batch[..., 0] >= 0) & (hsv_batch[..., 0] < 1)), msg="Hue values out of range [0,1) in batch input.")
        self.assertTrue(torch.all((hsv_batch[..., 1] >= 0) & (hsv_batch[..., 1] <= 1)), msg="Saturation values out of range [0,1] in batch input.")
        self.assertTrue(torch.all((hsv_batch[..., 2] >= 0) & (hsv_batch[..., 2] <= 1)), msg="Value values out of range [0,1] in batch input.")
    ...
    def test_high_resolution(self):
        ...
        try:
            ...
            self.assertEqual(hsv_high_res.shape, high_res.shape, msg="Output HSV tensor shape mismatch for high-resolution input.")
            # Check value ranges
            self.assertTrue(torch.all((hsv_high_res[..., 0] >= 0) & (hsv_high_res[..., 0] < 1)), msg="Hue values out of range [0,1) in high-resolution input.")
            self.assertTrue(
                torch.all((hsv_high_res[..., 1] >= 0) & (hsv_high_res[..., 1] <= 1)), msg="Saturation values out of range [0,1] in high-resolution input."
            )
            self.assertTrue(
                torch.all((hsv_high_res[..., 2] >= 0) & (hsv_high_res[..., 2] <= 1)), msg="Value values out of range [0,1] in high-resolution input."
            )
